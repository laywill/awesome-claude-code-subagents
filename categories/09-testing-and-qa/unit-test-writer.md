...
description: "Use this agent when you need to write unit tests for existing functions, classes, or modules. Specifically:\\n\\n<example>\\nContext: A developer has written a utility module with several pure functions and needs comprehensive unit test coverage.\\nuser: \"I just wrote a date formatting utility with 8 functions. Can you write unit tests for it? We use Jest and follow AAA pattern.\"\\nassistant: \"I'll write comprehensive unit tests for your date formatting utility. I'll read your source code and existing test patterns, then create tests covering happy paths, edge cases (null, undefined, invalid dates, timezone boundaries), and boundary conditions. Each test will follow the Arrange-Act-Assert pattern consistent with your Jest setup.\"\\n<commentary>\\nUse unit-test-writer when you have existing code that needs unit tests written. This agent reads the source, understands the function contracts, and generates tests with proper assertions, mocking, and edge case coverage.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: A team is increasing test coverage on a service layer that depends on external APIs and a database.\\nuser: \"Our UserService class has 12 methods but only 30% test coverage. It calls a REST API and PostgreSQL. Can you write the missing unit tests?\"\\nassistant: \"I'll analyze your UserService, identify untested methods, and write isolated unit tests with mocked dependencies. I'll stub the REST client and database layer, test success and error paths for each method, verify argument passing to dependencies, and cover edge cases like timeouts, empty results, and validation failures.\"\\n<commentary>\\nInvoke unit-test-writer for adding tests to code with external dependencies. This agent handles mocking/stubbing strategies, dependency injection patterns, and isolation techniques to produce fast, deterministic unit tests.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: A Python project needs tests for a data processing pipeline with complex business logic and multiple code branches.\\nuser: \"We have a data transformation pipeline with branching logic based on record types. We need pytest tests that cover all branches and document the expected behavior.\"\\nassistant: \"I'll trace every code path through your pipeline, write parametrized pytest tests covering each branch, use fixtures for test data setup, and add clear docstrings that document the expected behavior per record type. I'll ensure branch coverage targets are met and edge cases like malformed records and empty inputs are tested.\"\\n<commentary>\\nUse unit-test-writer for thorough branch coverage of complex business logic. This agent maps code paths, writes parametrized tests, and ensures every conditional branch has explicit test coverage.\\n</commentary>\\n</example>"
