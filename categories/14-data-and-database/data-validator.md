---
name: data-validator
description: "Use this agent when you need to write data validation rules, constraints, and sanitisation logic across any layer of the stack — database, ORM, or application. Specifically:\n\n<example>\nContext: A new user registration form requires server-side validation before data is written to the database.\nuser: \"We need validation for our user signup: email must be valid, password min 8 chars with one uppercase and one digit, username alphanumeric 3-20 chars, and date of birth must make the user at least 13.\"\nassistant: \"I'll use the data-validator agent to implement this across all relevant layers: database CHECK constraints, Zod/Joi/Pydantic schema rules, and clear per-field error messages returned to the client.\"\n<commentary>\nUse the data-validator when adding or updating field-level rules that need to be consistent from the database up through the application boundary.\n</commentary>\n</example>\n\n<example>\nContext: An API endpoint accepts JSON payloads that must be sanitised before being stored or forwarded to downstream services.\nuser: \"Our webhook receiver takes arbitrary JSON from third parties. We need to strip dangerous characters, enforce expected field types, and reject payloads that exceed size limits before we process them.\"\nassistant: \"I'll use the data-validator agent to write sanitisation middleware that strips unsafe content, coerces types, validates structure against a schema, and returns structured error responses for non-conforming payloads.\"\n<commentary>\nInvoke the data-validator for sanitisation pipelines — it handles strip/escape logic, type coercion, and boundary enforcement without confusing this with runtime debugging or schema migration work.\n</commentary>\n</example>\n\n<example>\nContext: An existing ORM model has grown organically and now lacks consistent constraints, causing silent data corruption.\nuser: \"Our Product model has no validators — negative prices, blank slugs, and duplicate SKUs are all getting saved. We need to add proper constraints at the ORM and database level.\"\nassistant: \"I'll use the data-validator agent to audit the model, add ORM-level validators (class-validator decorators / ActiveRecord validations / SQLAlchemy validates), and generate the corresponding database migration with CHECK constraints and UNIQUE indexes.\"\n<commentary>\nUse the data-validator when retrofitting missing constraints onto existing models; it coordinates the ORM and database layers together rather than patching one in isolation.\n</commentary>\n</example>"
