...
description: "Use this agent when you need to write, validate, or manage database migration scripts across any supported framework. Specifically:\n\n<example>\nContext: A new feature requires adding a nullable column to an existing high-traffic table without downtime.\nuser: \"I need to add a `subscription_tier` column to our `users` table. We use Alembic with PostgreSQL. Can you write the migration?\"\nassistant: \"I'll use the schema-migrator agent to generate an Alembic migration with a safe `ADD COLUMN` using a nullable column first, include a matching downgrade function, and validate backward compatibility before you apply it.\"\n<commentary>\nUse the schema-migrator agent whenever a schema change is needed. It writes both up and down migrations, checks for data-loss risk, and produces framework-correct file naming.\n</commentary>\n</example>\n\n<example>\nContext: A team is renaming a column and needs to keep the old name available during a rolling deployment.\nuser: \"We want to rename `user.email_address` to `user.email` in our Rails app. How do we do this safely with a zero-downtime migration?\"\nassistant: \"The schema-migrator agent will produce a multi-step migration plan: add the new column, backfill data, add an alias view or trigger for backward compatibility during rollout, then a follow-up migration to drop the old column once all app instances are updated.\"\n<commentary>\nInvoke the schema-migrator for non-trivial schema changes like column renames that require staged rollout strategies and backward-compatible intermediate states.\n</commentary>\n</example>\n\n<example>\nContext: A developer needs to audit whether pending migrations are safe to run before a release.\nuser: \"We have 5 pending Flyway migrations queued for tonight's release. Can you review them for data-loss risk and locking issues?\"\nassistant: \"I'll invoke the schema-migrator agent to read each pending migration file, check for destructive DDL (DROP, TRUNCATE, NOT NULL constraints on populated columns), identify statements that take long-lived locks, and produce a risk summary with recommended sequencing.\"\n<commentary>\nUse the schema-migrator to validate existing migration files for safety, not just to author new ones.\n</commentary>\n</example>"
