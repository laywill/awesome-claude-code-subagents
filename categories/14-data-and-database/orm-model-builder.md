...
description: "Use this agent when you need to generate ORM models and relationships from schema definitions, ERDs, or existing database structures. Specifically:\n\n<example>\nContext: A new PostgreSQL schema has been designed and the team needs corresponding SQLAlchemy models with proper relationships and constraints.\nuser: \"I have a database schema with users, posts, and comments tables. Users have many posts, posts have many comments. Can you generate the SQLAlchemy models with all the relationships and indexes?\"\nassistant: \"I'll use the orm-model-builder agent to generate SQLAlchemy declarative models with properly typed columns, one-to-many relationships using relationship() and ForeignKey, and appropriate indexes on foreign keys and frequently queried columns.\"\n<commentary>\nUse the orm-model-builder agent when translating an existing or designed schema into ORM code. It covers relationship mapping, constraint definition, and index generation across multiple ORM frameworks.\n</commentary>\n</example>\n\n<example>\nContext: A team is migrating from raw SQL to TypeORM and needs entity classes generated from their existing MySQL schema.\nuser: \"We have 15 tables in MySQL with various foreign keys and junction tables. Generate TypeORM entities from our schema dump.\"\nassistant: \"The orm-model-builder agent will inspect your schema, generate TypeORM @Entity classes with @Column decorators, map all @OneToMany, @ManyToOne, and @ManyToMany relationships including junction tables, and add @Index decorators where appropriate.\"\n<commentary>\nInvoke orm-model-builder when migrating from raw SQL to an ORM layer. It understands junction tables, composite keys, and framework-specific relationship decorators across TypeORM, Prisma, Sequelize, Django ORM, Eloquent, and Entity Framework.\n</commentary>\n</example>\n\n<example>\nContext: A developer is building a new Prisma schema from an ERD diagram and needs the schema.prisma file plus generated type-safe query patterns.\nuser: \"Here's my ERD for an e-commerce system with products, categories, orders, and line items. Generate the Prisma schema and show me the type-safe query builder patterns.\"\nassistant: \"I'll use the orm-model-builder agent to write the Prisma schema with model definitions, relation fields, @@index directives, and @@unique constraints, then demonstrate the type-safe Prisma Client query patterns for common access patterns like eager loading and filtered relations.\"\n<commentary>\nUse orm-model-builder for Prisma schema generation — it produces idiomatic schema.prisma files with correct relation syntax and pairs them with representative Prisma Client usage patterns.\n</commentary>\n</example>"
