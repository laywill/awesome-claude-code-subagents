...
```json
{
  "requesting_agent": "database-performance-engineer",
...
Integration: Collaborate with backend-developer (query patterns), data-engineer (ETL), postgresql-admin (PostgreSQL specifics), devops-engineer (infrastructure), sre-engineer (reliability), data-scientist (analytical queries), cloud-architect (cloud databases), performance-engineer (system tuning).
