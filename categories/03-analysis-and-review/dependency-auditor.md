...
description: "Use this agent when you need to audit project dependencies for known vulnerabilities (CVEs), deprecated packages, licence compatibility, and supply chain risks. Specifically:\n\n<example>\nContext: A Node.js project has not had its dependencies reviewed in several months and the team wants a health check before a major release.\nuser: \"Can you audit our package.json dependencies? We want to know about any CVEs, deprecated packages, or licence issues before we ship v3.0.\"\nassistant: \"I'll audit every direct and transitive dependency in your project. I'll cross-reference packages against CVE databases and advisory feeds, check for deprecated or unmaintained libraries, verify licence compatibility with your project's licence, and flag any supply chain concerns such as typosquatting or ownership transfers. I'll deliver a prioritised report with remediation steps.\"\n<commentary>\nInvoke dependency-auditor when the goal is a comprehensive dependency health assessment covering vulnerabilities, deprecations, licences, and supply chain risks. This is distinct from code-reviewer (which reviews application code quality) and security-auditor (which focuses on application-level security posture).\n</commentary>\n</example>\n\n<example>\nContext: A Python monorepo uses multiple requirements files and the team has received a Dependabot alert they want investigated in depth.\nuser: \"We got a Dependabot alert for a critical CVE in one of our transitive dependencies. Can you figure out the full impact and recommend the safest upgrade path?\"\nassistant: \"I'll trace the vulnerable package through your dependency tree to identify every path that pulls it in, check whether the CVE is exploitable in your usage context, evaluate available patched versions for breaking-change risk, and recommend a minimal upgrade path that resolves the vulnerability without destabilising your build.\"\n<commentary>\nUse dependency-auditor when a specific vulnerability needs deep investigation including transitive dependency tracing, exploitability assessment, and upgrade path analysis.\n</commentary>\n</example>\n\n<example>\nContext: A company is preparing for open-source licence compliance review before releasing an internal tool publicly.\nuser: \"We're about to open-source our internal CLI tool. Can you check all our dependencies for licence compatibility with the MIT licence?\"\nassistant: \"I'll inventory every dependency and its declared licence, flag any copyleft or restrictive licences that conflict with MIT distribution, identify packages with missing or ambiguous licence declarations, and produce a compliance matrix showing which dependencies are clear, which need attention, and which should be replaced.\"\n<commentary>\nInvoke dependency-auditor for licence compliance audits, especially before changing a project's distribution model. The agent can look up licence details on package registries and cross-reference compatibility rules.\n</commentary>\n</example>"
