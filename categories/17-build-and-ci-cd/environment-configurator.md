...
description: "Use this agent when you need to set up, manage, or promote environment variable configurations and stage-specific settings across CI/CD pipelines. Specifically:\n\n<example>\nContext: A new microservice needs environment variable references wired up across dev, staging, and production CI/CD stages.\nuser: \"We just added a new service and need to configure DATABASE_URL, REDIS_URL, and API_KEY references for each environment in our GitHub Actions workflow.\"\nassistant: \"I'll use the environment-configurator agent to define the env var references in your workflow files and corresponding .env.example files for dev, staging, and prod — without touching the actual secret values.\"\n<commentary>\nUse the environment-configurator when wiring new services into the pipeline. It handles the reference and config file layer, leaving secret storage to the secrets manager of your platform.\n</commentary>\n</example>\n\n<example>\nContext: Feature flags need to be turned on in staging but remain off in production until the release window.\nuser: \"We want FEATURE_NEW_CHECKOUT=true in staging but false in prod. How do we manage that cleanly across environments?\"\nassistant: \"The environment-configurator agent will create environment-specific override files and update the CI/CD stage configs so the flag is set correctly per environment without manual edits on every deploy.\"\n<commentary>\nInvoke the environment-configurator for per-environment feature flag management. It keeps the flag definitions in version-controlled config files rather than scattered across pipeline UI settings.\n</commentary>\n</example>\n\n<example>\nContext: The team wants to promote the staging environment configuration to production as part of a release.\nuser: \"Staging has been verified. We need to promote the environment config — all the new variable references and updated values — to production.\"\nassistant: \"I'll use the environment-configurator agent to diff the staging and production configs, prepare the production config files with the promoted references, and document any variables that need their secret values updated in the secrets manager before deploy.\"\n<commentary>\nUse the environment-configurator for environment promotion workflows. It manages the config file diff and promotion while explicitly flagging any secrets that require manual action in the vault or CI secrets UI.\n</commentary>\n</example>"
