...
description: "Use this agent when you need to build a wrapper library around a third-party SDK, isolate vendor dependencies behind an abstraction layer, or add resilience patterns (retry, timeout, circuit breaker) on top of an external client. Specifically:\n\n<example>\nContext: The team is adopting a new payment provider SDK and wants to ensure they can swap vendors later without touching business logic.\nuser: \"We're integrating Stripe's SDK throughout the checkout flow but want to avoid hard-coding Stripe everywhere. Can you build a wrapper?\"\nassistant: \"I'll use the sdk-wrapper-builder agent to design a payment-provider interface, implement a Stripe adapter behind it, add retry and timeout logic, and wrap Stripe errors into our own error hierarchy so the rest of the codebase stays vendor-neutral.\"\n<commentary>\nUse this agent whenever direct SDK usage would spread vendor-specific types and error codes across the codebase. The wrapper isolates the vendor surface so a future migration touches only the adapter, not every call site.\n</commentary>\n</example>\n\n<example>\nContext: An internal service calls an AWS SDK directly in dozens of places and has no consistent error handling or retry behaviour.\nuser: \"Our S3 calls are scattered everywhere and each team handles errors differently. We need a unified S3 client wrapper.\"\nassistant: \"The sdk-wrapper-builder agent will audit all existing S3 call sites, define a typed StorageClient interface, build an S3Adapter that applies exponential-backoff retries, maps AWS errors to our internal error types, and replaces raw SDK usage at each call site.\"\n<commentary>\nInvoke this agent when you want to retrofit resilience patterns and a consistent error model onto an existing SDK dependency that is already widely used.\n</commentary>\n</example>\n\n<example>\nContext: A new third-party analytics SDK needs to be integrated, but the team wants typed interfaces and a test double for unit testing.\nuser: \"We need to add Segment analytics but I don't want real network calls in our test suite and I want TypeScript types for every event.\"\nassistant: \"I'll use the sdk-wrapper-builder agent to create a typed AnalyticsClient interface with a SegmentAdapter for production and an InMemoryAdapter for tests, so event schemas are enforced at compile time and tests run without network access.\"\n<commentary>\nUse this agent when testability and type safety are primary concerns. The wrapper pattern enables in-memory or mock adapters that satisfy the same interface as the real SDK adapter.\n</commentary>\n</example>"
