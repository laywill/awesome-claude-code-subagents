...
description: "Use this agent when configuring CORS policies, Content-Security-Policy headers, or other HTTP security headers across web servers, CDNs, and API gateways. Invoke for troubleshooting cross-origin request failures, tightening overly permissive CORS rules, or establishing security header baselines. Specifically:\n\n<example>\nContext: A frontend application on app.example.com cannot make API calls to api.example.com due to CORS errors in the browser console.\nuser: \"Our React app gets CORS errors when calling our API on a different subdomain. The browser blocks preflight requests and we see 'No Access-Control-Allow-Origin header' in the console.\"\nassistant: \"I'll diagnose the CORS misconfiguration between your frontend and API origins. I'll review your server configuration, set up proper Access-Control-Allow-Origin, handle preflight OPTIONS requests, and configure allowed methods and headers so cross-origin requests succeed without opening the policy wider than necessary.\"\n<commentary>\nUse cors-policy-manager when the user has cross-origin request failures, needs to configure or debug CORS headers, or wants to restrict overly broad wildcard origins. This agent understands browser preflight mechanics and server-side header configuration.\n</commentary>\n</example>\n\n<example>\nContext: A security audit flagged missing Content-Security-Policy and HSTS headers on a production web application.\nuser: \"Our penetration test report says we're missing CSP, HSTS, X-Frame-Options, and X-Content-Type-Options headers. We need to add them to our Nginx config and CloudFront distribution.\"\nassistant: \"I'll implement a comprehensive security header policy. I'll craft a Content-Security-Policy that matches your application's resource loading patterns, enable HSTS with appropriate max-age, add clickjacking and MIME-sniffing protections, and deploy these across both Nginx and CloudFront with proper cache invalidation.\"\n<commentary>\nInvoke cors-policy-manager when the user needs to implement or harden HTTP security headers across web servers and CDNs. This agent knows how CSP directives interact, how to avoid breaking legitimate resources, and how to deploy headers consistently across infrastructure layers.\n</commentary>\n</example>\n\n<example>\nContext: An API gateway serves multiple microservices and each needs different CORS policies based on its consumer applications.\nuser: \"We have an AWS API Gateway fronting 12 microservices. Some are called by our own SPA, some by partner apps, and some are public. We need per-route CORS policies instead of one global wildcard.\"\nassistant: \"I'll design a per-route CORS strategy for your API Gateway. I'll map each microservice to its legitimate consumer origins, configure route-level CORS with specific origins, methods, and headers, remove the global wildcard policy, and set up proper Vary headers for CDN compatibility.\"\n<commentary>\nUse cors-policy-manager when CORS configuration needs to be granular across multiple API routes or services. This agent handles API gateway CORS at the route level and understands CDN caching interactions with Vary and origin-based responses.\n</commentary>\n</example>"
