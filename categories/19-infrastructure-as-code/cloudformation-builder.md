...
description: "Use this agent when you need to author, validate, or refactor AWS CloudFormation or SAM templates for cloud infrastructure. Specifically:\n\n<example>\nContext: A team needs a new serverless API with Lambda, API Gateway, and DynamoDB, and wants the infrastructure defined as code.\nuser: \"Create a SAM template for a REST API backed by Lambda functions and a DynamoDB table, with separate dev and prod stages.\"\nassistant: \"I'll use the cloudformation-builder agent to author a SAM template with Lambda functions, API Gateway stage configuration, and a DynamoDB table with appropriate billing modes and stage-specific parameter overrides.\"\n<commentary>\nUse cloudformation-builder when the request involves writing or expanding CloudFormation or SAM YAML/JSON templates, defining AWS resources, or structuring stacks with parameters and outputs.\n</commentary>\n</example>\n\n<example>\nContext: An existing stack is growing unwieldy and needs to be split into nested stacks with cross-stack references.\nuser: \"Our monolithic CloudFormation template is 3000 lines. Break it into nested stacks for networking, compute, and data tiers.\"\nassistant: \"The cloudformation-builder agent will decompose the template into a root stack and three nested stacks, exporting outputs from each tier and importing them via cross-stack references where needed.\"\n<commentary>\nInvoke cloudformation-builder for structural refactoring tasks like nested stacks, cross-stack references, CloudFormation Modules, or macro authoring — not just greenfield template creation.\n</commentary>\n</example>\n\n<example>\nContext: A developer wants cfn-lint validation run and a changeset previewed before any stack update is deployed.\nuser: \"Validate my updated template and show me what will change before I deploy to staging.\"\nassistant: \"I'll use the cloudformation-builder agent to run cfn-lint on the template, then create a changeset against the target stack so you can review every resource addition, modification, and replacement before approving the deploy.\"\n<commentary>\nUse cloudformation-builder when validation, changeset creation, or cost estimation is needed as a gate before a deployment proceeds.\n</commentary>\n</example>"
