...
description: "Use this agent when you need to write or improve Dockerfiles, docker-compose configurations, or related Docker artifacts such as .dockerignore files, health checks, volume mounts, and service networking. Specifically:\n\n<example>\nContext: A Node.js application needs a production-ready Dockerfile with multi-stage builds to minimise image size and a docker-compose.yml for local development.\nuser: \"Set up Docker for our Express API. We want a small production image and a local dev compose file with hot reload.\"\nassistant: \"I'll use the docker-composer agent to write a multi-stage Dockerfile that separates build and runtime stages, then create a docker-compose.yml with a bind-mounted volume for hot reload, health checks, and a non-root user for security hardening.\"\n<commentary>\nUse the docker-composer agent whenever you need to create or refactor Dockerfiles and compose files. It handles multi-stage builds, layer optimisation, security hardening, and local vs CI compose configurations.\n</commentary>\n</example>\n\n<example>\nContext: An existing Dockerfile runs as root, has large image size due to poor layer ordering, and the team wants it hardened before going to production.\nuser: \"Our Docker image is 1.4 GB and runs as root. Can you fix it?\"\nassistant: \"The docker-composer agent will audit the Dockerfile, reorder COPY/RUN layers for optimal cache reuse, switch to a minimal base image (distroless or alpine), add a non-root USER directive, and lint with hadolint to catch remaining issues.\"\n<commentary>\nInvoke docker-composer for security hardening tasks: switching to non-root users, adopting minimal base images, reducing attack surface, and eliminating unnecessary packages.\n</commentary>\n</example>\n\n<example>\nContext: A microservices project needs a docker-compose.yml that wires together a web service, a Postgres database, and a Redis cache with proper networking and persistent volumes.\nuser: \"Write a compose file for our web app, Postgres, and Redis. They need to talk to each other but Redis shouldn't be exposed externally.\"\nassistant: \"I'll use the docker-composer agent to define a compose file with a custom bridge network, expose Postgres and Redis only on the internal network, mount named volumes for data persistence, and add health checks so the web service waits for dependencies to be ready.\"\n<commentary>\nUse docker-composer when networking, volume management, or service dependency ordering in docker-compose is needed.\n</commentary>\n</example>"
