---
name: bug-fixer
description: "Use this agent when you need to diagnose a reported bug, reproduce it, implement a fix, and write regression tests to prevent recurrence. Specifically:\\n\\n<example>\\nContext: A user reports that a checkout form silently swallows errors and submits nothing when the card number is invalid.\\nuser: \"Users are saying the checkout button does nothing when they type a bad card number. No error shown, no network request. Bug report attached.\"\\nassistant: \"I'll use the bug-fixer agent to reproduce the failure from the bug report, trace through the form submission code to find why invalid card input is suppressed, implement the fix, and add a regression test so this path is covered going forward.\"\\n<commentary>\\nUse the bug-fixer agent when a specific bug has been reported and the goal is a complete fix cycle: reproduction, root cause, patch, and regression test. The debugger agent diagnoses unknowns; the bug-fixer delivers the resolved ticket.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: A backend service returns HTTP 500 on certain date inputs. The issue is reproducible with a known input but the cause is unclear.\\nuser: \"Passing a date string like '2024-02-29' to /api/events throws a 500. Normal dates work fine. Here's the stack trace.\"\\nassistant: \"I'll invoke the bug-fixer agent to reproduce the 500 with that input, locate the date parsing code, identify the edge case (likely leap-year handling), patch it, and add a parameterised test covering boundary dates.\"\\n<commentary>\\nInvoke the bug-fixer when you have a reproducible input, a stack trace or error description, and want a full fix delivered — not just a diagnosis.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: A recently merged PR broke pagination — page 2 always returns the same results as page 1.\\nuser: \"Since the refactor landed yesterday, pagination is broken. Page 2 shows the same items as page 1 every time.\"\\nassistant: \"I'll use the bug-fixer agent to check the recent diff, reproduce the pagination bug, identify the offset/limit regression introduced in the refactor, apply the fix, and write a regression test for page-boundary correctness.\"\\n<commentary>\\nUse the bug-fixer when a regression has been introduced by a specific change and you need the bug reproduced, fixed, and guarded against with a test — not just investigated.\\n</commentary>\\n</example>"
